...
Otherwise, you need to download the Neo4J Community Edition (you must use version 2.1.x) http://neo4j.com/download/[here].
...
. Next, name your database, choose Neo4J Community Edition 2.1.8, and click on ``Create database'':
+
...
. Visit the Neo4J web admin console for your GrapheneDB instance (you will find a *Launch* button at the bottom of the page https://app.graphenedb.com/dbs/name_you_assigned_your_db/overview). Run the following Cypher queries to load test data:
+
----
LOAD CSV WITH HEADERS FROM "https://raw.githubusercontent.com/mstine/CloudNativeArchitectureClass/citi/day_01/session_04/lab_13/movies.csv" AS row CREATE (movie:Movie:_Movie {mlId:row.mlId, title:row.title})
...
+
----
cf cups spring-recommendations-db -p '{\"neo4jUri\":\"http://<username>:<password>@<host>\"}'
----
