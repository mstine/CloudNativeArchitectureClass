...
== Optional Health Management Exercise

. Scale the application back to more than 1 instance.

. Use the "Kill Switch" button on the application to kill an instance.

. Inspect the Application Dashboard for the "app crashed" events.

. Notice that Pivotal Cloud Foundry automatically starts a new instance to compensate for any failed instancess and there is no downtime throughout the process.
